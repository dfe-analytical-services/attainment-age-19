...
la_plot_rate_fsm <- function(la, category) {
  ...
  if (category == "Level 2 with English & maths") {
    ...
    d <- d %>%
      mutate(y_var = l2_with_em_by19_rate) %>%
      filter(y_var != "x")
  }
  ...
  if (category == "Level 3") {
    ...
    d <- d %>%
      mutate(y_var = l3_by19_rate) %>%
      filter(y_var != "x")
  }
  d <- d %>%
    rename(`Cohort 19 in` = cohort_19_in, Percentage = y_var, FSM = fsm) %>%
    mutate(
...
la_plot_num_fsm <- function(la, category) {
  ...
  if (category == "Level 3") {
    ...
    d <- d %>%
      mutate(y_var = l3_by19) %>%
      filter(y_var != "c")
  }
  d <- d %>%
    rename(`Cohort 19 in` = cohort_19_in, Number = y_var, FSM = fsm) %>%
    mutate(
...
la_plot_data_sen <-
  dplyr::select(
    filter(
      la_ud, la_name != "ALL", gender == "ALL", fsm == "ALL", cohort_19_in != first_year, cohort_19_in <= latest_year,
...
la_plot_rate_sen <- function(la2, category2) {
  ...
  d <- d %>%
    rename(`Cohort 19 in` = cohort_19_in, Percentage = y_var, SEN = sen) %>%
    mutate(
      Percentage = round(as.numeric(Percentage), 1),
      FSM = as.factor(SEN)
    )
...
la_plot_num_sen <- function(la2, category2) {
  ...
  d <- d %>%
    rename(`Cohort 19 in` = cohort_19_in, Number = y_var, SEN = sen) %>%
    mutate(
      Number = round(as.numeric(Number), 1),
      FSM = as.factor(SEN)
    )
