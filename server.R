...
server <- function(input, output, session) {
  ...
  reactiveFSM_All <- reactive({
    la_ud_VB %>% filter(
      la_name == input$select2, category == input$select_cat,
      fsm == "ALL", sen == "ALL"
    )
  })
  ...
  output$boxFSM_All <- renderValueBox({
    ...
    valueBox(
      ...
      HTML(paste0("of all pupils achieved ", input$select_cat, " by age 19", br())),
      color = "blue"
    )
  })
  ...
  reactiveSEN_All <- reactive({
    la_ud_VB %>% filter(
      la_name == input$select3, category == input$select_cat2,
      fsm == "ALL", sen == "ALL"
    )
  })
  ...
  output$boxSEN_All <- renderValueBox({
    ...
    valueBox(
      # take input number
      paste0((format(
        (reactiveSEN_All() %>% filter(
          la_name == input$select3,
          category == input$select_cat2
        ))$value, # "%",
        big.mark = ","
      )), "%"),
      # add subtitle to explain what it's showing
      HTML(paste0("of all pupils achieved ", input$select_cat2, " by age 19", br(), br())),
      color = "blue"
    )
  })
  ...
  output$boxSEN_No <- renderValueBox({
    ...
    valueBox(
      ...
      # add subtitle to explain what it's showing
      HTML(paste0("of pupils with no identified SEN achieved ", input$select_cat2, " by age 19", br())),
      color = "blue"
    )
  })
  ...
  output$boxSEN_with <- renderValueBox({
    ...
    valueBox(
      ...
      # add subtitle to explain what it's showing
      paste0("of pupils with SEN with statements or EHC plans achieved ", input$select_cat2, " by age 19"),
      color = "blue"
    )
  })
  ...
  output$boxSEN_without <- renderValueBox({
    ...
    valueBox(
      ...
      # add subtitle to explain what it's showing
      paste0("of pupils with SEN without statements or EHC plans achieved ", input$select_cat2, " by age 19"),
      color = "blue"
    )
  })
  ...
  output$la_title <- renderText({
    paste(input$select2, " summary")
  })
  ...
  output$la_sum_fsm <- renderText({
    if (TRUE %in% (any(la_table_num_fsm(input$select2, input$select_cat)[, 2:max(ncol(la_table_num_fsm(input$select2, input$select_cat)))] == "c") |
      ...
      } else if (input$select_cat == "Level 2 with English & maths") {
        paste(
      ...
      } else if (input$select_cat == "Level 3") {
        paste(
          "The percentage achieving level 3 by 19 in ", input$select3, " ", change_ed(round(as.numeric(la_l3_rate(input$select2, last_year)), 1), round(as.numeric(la_l3_rate(input$select2, latest_year)), 1)),
  ...
  # LA summary text - SEN.
  # Gives the latest year changes depending on what measure is selected.
  output$la_title2 <- renderText({
    paste(input$select3, " summary")
  })
  ...
  output$la_sum_sen <- renderText({
    if (TRUE %in% (any(la_table_num_sen(input$select3, input$select_cat2)[, 2:max(ncol(la_table_num_sen(input$select3, input$select_cat2)))] == "c") |
      ...
      if (input$select_cat2 == "Level 2") {
        paste(
          "The percentage achieving level 2 by 19 in ", input$select3, " ", change_ed(round(as.numeric(la_l2_rate2(input$select3, last_year)), 1), round(as.numeric(la_l2_rate2(input$select3, latest_year)), 1)),
          "", round(as.numeric(la_l2_rate2(input$select3, last_year)), 1),
      ...
      else if (input$select_cat2 == "Level 2 with English & maths") {
        paste(
          "The percentage achieving level 2 with English and maths by 19 in ", input$select3, " ", change_ed(round(as.numeric(la_l2em_rate2(input$select3, last_year)), 1), round(as.numeric(la_l2em_rate2(input$select3, latest_year)), 1)),
      ...
      } else if (input$select_cat2 == "Level 3") {
        paste(
          "The percentage achieving level 3 by 19 in ", input$select3, " ", change_ed(round(as.numeric(la_l3_rate2(input$select3, last_year)), 1), round(as.numeric(la_l3_rate2(input$select3, latest_year)), 1)),
  ...
  output$region_title <- renderText({
    paste(str_sub(region_name(input$select2), 1, 25), " region summary")
  })

  output$reg_sum_fsm <- renderText({
    if (input$select_cat == "Level 2") {
      paste(
        "For those receiving free school meals, ",
  ...
  # regional text - SEN
  output$region_title2 <- renderText({
    paste(str_sub(region_name2(input$select3), 1, 25), " region summary")
  })
  ...
  output$nat_sum_fsm <- renderText({
    if (input$select_cat == "Level 2") {
      paste(
        "In comparison, in the ", str_sub(region_name(input$select2), 1, 25), "region,",
  ...
  output$fsm_la_data <- DT::renderDataTable(
    {
      fsm_la_table(input$select2)
    },
  ...
  output$sen_la_data <- DT::renderDataTable(
    {
      sen_la_table(input$select3)
    },
    extensions = "FixedColumns",
    options = list(
      paging = FALSE,
